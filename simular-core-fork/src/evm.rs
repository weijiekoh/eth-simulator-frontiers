...
/// Serializable representation of BaseEvm state
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct SerializableEvm {
    /// Snapshot of all account and storage data
    ...
    /// Chain ID
    pub chain_id: u64,
    /// Fork URL (if this was a forked EVM)
...
impl BaseEvm {
    ...
    /// Fetches proof from provider on first access to verify authenticity.
    pub fn get_balance(&mut self, caller: Address) -> Result<U256> {
        // First try to get current balance from EVM state (includes any local changes)
        ...
        // If we have a fork and this is the first time accessing this account,
        // fetch and store the proof for initial verification
        if let Some(fork) = self.backend.get_fork_mut() {
            // Only fetch proof if we don't already have account record stored
            if !fork.account_records.contains_key(&caller) {
                // Always expect to get either an existence proof or nonexistence proof
                // RPC should never fail for proof requests - it should return nonexistence proofs
                match fork.db.db.fetch_basic_from_fork(caller) {
                    Ok((account_info, proof)) => {
                        // Store the account record with proof for later verification
                        // This includes both existence proofs and nonexistence proofs
                        fork.store_account_with_proof(caller, &account_info, proof);
                    }
                    Err(e) => {
                        // This should never happen - RPC should always provide proofs
                        // But if it does, we cannot continue safely
                        return Err(anyhow!("Failed to fetch proof for account {}: {:?}. RPC provider should always return either existence or nonexistence proofs.", caller, e));
                    }
                }
            }
        }
    ...
    pub fn get_current_block_hash(&mut self) -> Result<alloy_primitives::FixedBytes<32>> {
        use revm::Database;
        let block_num = U256::from(self.backend.block_number);
        self.backend.block_hash(block_num)
    ...
    /// Verify account proofs for all accounts with stored proof data.
    /// Returns a list of tuples (Address, ProofVerificationResult) indicating verification results.
    /// The state root is taken from the provided block header.
    /// This method uses proofs that were stored when accounts were fetched.
    pub fn verify_proofs(&self, block_header: &ethers_core::types::Block<ethers_core::types::TxHash>) -> Result<Vec<(Address, ProofVerificationResult)>> {
        // Check if we have a fork
        let fork = match self.backend.get_fork() {
            Some(fork) => fork,
            None => bail!("verify_proofs requires a forked backend with provider access"),
        };

        // Get the state root from the provided block header
        let state_root = block_header.state_root;

        // Collect addresses from stored account records
        let account_records = &fork.account_records;

        if account_records.is_empty() {
            return Ok(Vec::new());
        }

        // Collect verification results for all stored account records
        let mut verification_results = Vec::new();

        // Iterate through all stored account records and verify their proofs
        for (address, account_record) in account_records {
            // Perform verification using stored proof
            // This works for both existence proofs and nonexistence proofs
            let result = match self.verify_single_account_proof(*address, &account_record.proof, state_root) {
                Ok(()) => ProofVerificationResult::Valid,
                Err(_) => ProofVerificationResult::Invalid,
            };
            
            verification_results.push((*address, result));
        }

        Ok(verification_results)
    }

    /// Verify a single account proof against the given state root
    fn verify_single_account_proof(
        &self,
        address: Address,
        account_proof: &ethers_core::types::EIP1186ProofResponse,
        state_root: ethers_core::types::H256,
    ) -> Result<()> {
        use rlp::RlpStream;

        // Create the expected account data RLP encoding
        // Account structure: [nonce, balance, storage_root, code_hash]
        let mut account_rlp = RlpStream::new_list(4);
        account_rlp.append(&account_proof.nonce);
        account_rlp.append(&account_proof.balance);
        account_rlp.append(&account_proof.storage_hash);
        account_rlp.append(&account_proof.code_hash);
        let expected_account_rlp = account_rlp.out().to_vec();

        // Use the existing verify_account_proof function from reference code
        self.verify_account_proof_internal(address, account_proof, state_root, expected_account_rlp)
    }

    /// Internal verification function using alloy_trie (when dependencies are aligned)
    /// For now, this is a placeholder that returns success for testing
    fn verify_account_proof_internal(
        &self,
        address: Address,
        account_proof: &ethers_core::types::EIP1186ProofResponse,
        _state_root: ethers_core::types::H256,
        expected_account_rlp: Vec<u8>,
    ) -> Result<()> {
        // TODO: Once alloy_primitives version conflicts are resolved, implement:
        // let key = keccak256(address);
        // let verification_result = verify_proof(
        //     state_root.into(),
        //     Nibbles::unpack(&key),
        //     Some(expected_account_rlp),
        //     &account_proof.account_proof.iter().map(|b| Bytes::from(b.0.clone())).collect::<Vec<_>>(),
        // );
        
        // For now, perform basic validation
        if account_proof.account_proof.is_empty() {
            return Err(anyhow!("Account proof is empty for address: {}", address));
        }
        
        // Basic sanity checks - just validate that fields are present
        // (U256 and U64 are always >= 0, so just check for existence)
        if expected_account_rlp.is_empty() {
            return Err(anyhow!("Failed to encode account RLP for address: {}", address));
        }
    ...
    /// Convert the EVM to a serializable representation
    pub fn to_serializable(&self) -> Result<SerializableEvm> {
        ...
        Ok(SerializableEvm {
            ...
            chain_id: self.env.cfg.chain_id,
            fork_url: self.backend.get_fork_url(),
        })
    }

    /// Create an EVM from a serializable representation
    pub fn from_serializable(serializable: SerializableEvm) -> Result<Self> {
        let fork = if let Some(fork_url) = &serializable.fork_url {
            Some(CreateFork {
                url: fork_url.clone(),
                blocknumber: Some(serializable.block_number),
            })
        } else {
            None
        };

        let mut evm = BaseEvm::new(fork);
        
        // Set the timestamp and block number
        evm.backend.timestamp = serializable.timestamp;
        evm.backend.block_number = serializable.block_number;
        
        // Set the chain ID
        evm.env.cfg.chain_id = serializable.chain_id;
        
        // Restore all account data from snapshot
        evm.backend.restore_from_snapshot(&serializable.snapshot)?;

        Ok(evm)
    }

    /// Serialize the EVM to a JSON string
    pub fn to_json(&self) -> Result<String> {
        let serializable = self.to_serializable()?;
        serde_json::to_string(&serializable).map_err(|e| anyhow!("Failed to serialize EVM: {}", e))
    }

    /// Deserialize the EVM from a JSON string
    pub fn from_json(json: &str) -> Result<Self> {
        let serializable: SerializableEvm = serde_json::from_str(json)
            .map_err(|e| anyhow!("Failed to deserialize EVM: {}", e))?;
        Self::from_serializable(serializable)
    }
}

/// Result of account proof verification
/// Either the proof is valid or invalid - there should always be a proof
/// (either existence proof for existing accounts or nonexistence proof for non-existing accounts)
