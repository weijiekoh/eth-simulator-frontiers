...
use anyhow::Result;
use ethers_core::types::{Block, BlockId, BlockNumber, TxHash, H160, H256, U64, EIP1186ProofResponse};
...
impl ForkBackend {
    ...
    pub fn fetch_basic_from_fork(&self, address: Address) -> Result<(AccountInfo, EIP1186ProofResponse), ProviderError> {
        ...
        let f = async {
            ...
            let proof = self.provider.get_proof(add, vec![], bn);
            tokio::join!(nonce, balance, code, proof)
        };
        let (nonce, balance, code, proof) = Self::block_on(f);

        let balance = U256::from_limbs(balance?.0);
        let nonce = nonce?.as_u64();
        let bytecode = Bytecode::new_raw(code?.0.into());
        let code_hash = bytecode.hash_slow();
        let account_info = AccountInfo::new(balance, nonce, code_hash, bytecode);
        
        // For proof, we must always get either an existence proof or nonexistence proof
        // If get_proof fails, that's a real error that should be propagated
        let proof_response = proof?;
        
        // Assert that the proof is not empty
        if proof_response.account_proof.is_empty() {
            return Err(ProviderError::CustomError("Proof data is empty - this should never happen".into()));
        }
        
        Ok((account_info, proof_response))
    }
