...
impl StorageBackend {
    ...
    /// Get reference to the fork database if available
    pub fn get_fork(&self) -> Option<&Fork> {
        self.forkdb.as_ref()
    }

    /// Get mutable reference to the fork database if available
    pub fn get_fork_mut(&mut self) -> Option<&mut Fork> {
        self.forkdb.as_mut()
    }
}
...
impl StorageBackend {
    /// Get the fork URL if this backend is using a fork
    pub fn get_fork_url(&self) -> Option<String> {
        self.forkdb.as_ref().map(|fork| fork.db.db.url.clone())
    }

    /// Restore state from a snapshot
    pub fn restore_from_snapshot(&mut self, snapshot: &crate::SnapShot) -> anyhow::Result<()> {
        // Clear existing state - we'll use a simplified approach
        // Just load the snapshot using existing infrastructure
