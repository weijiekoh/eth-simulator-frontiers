...
pub struct Fork {
    ...
    pub timestamp: u64,
    pub account_records: BTreeMap<Address, SnapShotAccountRecord>,
}
...
impl Fork {
    ...
    /// Store account data with proof when fetched from RPC
    pub fn store_account_with_proof(&mut self, address: Address, account_info: &AccountInfo, proof: ethers_core::types::EIP1186ProofResponse) {
        // Get bytecode
        let code = if let Some(code) = &account_info.code {
            code.original_bytes()
        } else {
            // Try to fetch from cache or use empty
            match self.database().code_by_hash_ref(account_info.code_hash) {
                Ok(bytecode) => bytecode.original_bytes(),
                Err(_) => alloy_primitives::Bytes::new(),
            }
        };

        let account_record = SnapShotAccountRecord {
            nonce: account_info.nonce,
            balance: account_info.balance,
            code,
            storage: BTreeMap::new(), // Will be populated when storage is accessed
            proof,
        };

        self.account_records.insert(address, account_record);
    }
        ...
        let accounts = self
            ...
            .map(
                |(k, v)| -> anyhow::Result<(Address, SnapShotAccountRecord)> {
                    ...
                    // Fetch proof from the fork backend
                    let proof = match self.database().db.fetch_basic_from_fork(k) {
                        Ok((_, proof)) => proof,
                        Err(_) => {
                            // This should not happen if the RPC provider is working correctly
                            return Err(anyhow::anyhow!("Failed to fetch proof for account {} during snapshot creation", k));
                        }
                    };
